...
export default function SettingsPage() {
  ...
  const sections = useMemo(
    ...
    [isDarkMode, isNotificationsEnabled, selectedLanguage]
  );
              ...
              <View className={`${cardBgClass} border rounded-2xl overflow-hidden`}>
                {section.items.map((item, index) => (
                  ...
                  >
                    ...
                    {item.type === "toggle" ? (
                      <View
                        className={`w-12 h-6 rounded-full ${item.enabled ? "bg-green-500" : isDarkMode ? "bg-gray-700" : "bg-gray-300"}`}
                      >
                        <View
                          className={`w-5 h-5 bg-white rounded-full mt-0.5 ${item.enabled ? "ml-6" : "ml-0.5"}`}
                        />
                      </View>
                    ) : item.value ? (
