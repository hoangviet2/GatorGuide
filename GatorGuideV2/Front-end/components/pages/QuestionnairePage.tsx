...
export default function QuestionnairePage() {
  ...
  const progressBgClass = isDarkMode ? "bg-gray-900" : "bg-gray-200";
      ...
      <View className="max-w-md w-full self-center">
        <View className="px-6 pt-8 pb-4 flex-row items-center">
          <Pressable onPress={handleBack} className="mr-4">
            <MaterialIcons name="arrow-back" size={24} color={isDarkMode ? "#9CA3AF" : "#6B7280"} />
          </Pressable>
          <View className="flex-1">
            <Text className={`text-xl ${textClass}`}>Detailed Questionnaire</Text>
            <Text className={`text-sm ${secondaryTextClass}`}>Step {currentStep + 1} of {questions.length}</Text>
          </View>
        </View>
